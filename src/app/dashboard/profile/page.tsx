...
import { User, Mail, MapPin, Phone, Calendar, Edit, Camera, Award, BookOpen, Clock, TrendingUp, Shield, MoreHorizontal, Settings, LogOut } from "lucide-react";
      ...
      <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 overflow-hidden relative group">
        ...
        <div className="px-8 pb-8">
            ...
            <div className="relative group">
              <div className="w-32 h-32 rounded-[2rem] border-4 border-white bg-white shadow-xl flex items-center justify-center text-blue-600 text-4xl font-bold overflow-hidden">
                {session?.user?.image ? (
                   ...
                   <span className="bg-blue-50 w-full h-full flex items-center justify-center">{session?.user?.name?.[0]?.toUpperCase() || "U"}</span>
                )}
              </div>
              <button 
                type="button"
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            {[
              { label: "Target Band", value: userData.targetScore, icon: <TargetIcon />, color: "text-blue-600", bg: "bg-blue-50" },
              { label: "Tests Taken", value: "12", icon: <BookOpen size={20} />, color: "text-green-600", bg: "bg-green-50" },
              { label: "Practice Time", value: "24.5h", icon: <Clock size={20} />, color: "text-purple-600", bg: "bg-purple-50" },
              { label: "Success Rate", value: "85%", icon: <TrendingUp size={20} />, color: "text-orange-600", bg: "bg-orange-50" },
            ].map((stat, i) => (
              <div key={i} className="p-4 rounded-2xl bg-slate-50 border border-slate-100 flex items-center gap-4 hover:bg-white hover:shadow-md transition-all duration-300">
        ...
        <div className="lg:col-span-1 space-y-8">
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
            ...
            <div className="space-y-5">
              <InfoItem icon={<Mail size={18} />} label="Email" value={session?.user?.email} />
              <InfoItem icon={<Phone size={18} />} label="Phone" value={userData.phone} />
              <InfoItem icon={<Calendar size={18} />} label="Joined" value={userData.joinDate} />
              <InfoItem icon={<Shield size={18} />} label="Exam Date" value={userData.nextExamDate} />
            </div>
          </div>
          ...
          <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8">
            ...
            <div className="space-y-6">
              <SkillBar label="Listening" score="7.5" color="bg-blue-500" width="85%" />
              <SkillBar label="Reading" score="7.0" color="bg-green-500" width="78%" />
              <SkillBar label="Writing" score="6.5" color="bg-yellow-500" width="72%" />
              <SkillBar label="Speaking" score="6.5" color="bg-orange-500" width="72%" />
            </div>
          </div>
        </div>
          ...
          {isEditing ? (
            ...
            >
              ...
              <form className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <InputGroup label="Full Name" defaultValue={session?.user?.name} />
                  <InputGroup label="Email" defaultValue={session?.user?.email} disabled />
                  <InputGroup label="Phone" defaultValue={userData.phone} />
                  <InputGroup label="Location" defaultValue={userData.location} />
                  
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Target Score</label>
                  ...
                  <InputGroup label="Next Exam Date" type="date" defaultValue={userData.nextExamDate} />
                </div>
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-8 h-full flex flex-col">
              <div className="flex items-center justify-between mb-8">
                 ...
                 <button type="button" className="text-sm font-bold text-blue-600 hover:text-blue-700 hover:underline">View All</button>
              </div>
              ...
              <div className="space-y-2 flex-1">
                {[
                  ...
                  { title: "Completed Reading Mock Test 3", date: "5 days ago", score: "6.5", type: "Test" }
                ].map((activity, i) => (
                  <div key={i} className="group flex items-center gap-4 p-4 rounded-2xl hover:bg-slate-50 transition-colors border border-transparent hover:border-slate-100">
                    <div className={`p-3 rounded-xl shrink-0 ${
                      activity.type === 'Test' ? 'bg-blue-100 text-blue-600' :
                      activity.type === 'Practice' ? 'bg-green-100 text-green-600' :
                      'bg-purple-100 text-purple-600'
                    }`}>
                      {activity.type === 'Test' ? <Award className="w-5 h-5" /> :
                       activity.type === 'Practice' ? <BookOpen className="w-5 h-5" /> :
                       <User className="w-5 h-5" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-base font-bold text-slate-900 group-hover:text-blue-600 transition-colors truncate">{activity.title}</p>
                      <p className="text-xs text-slate-500 mt-1 font-medium">{activity.date}</p>
                    </div>
                    {activity.score && (
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-bold ${
                        activity.score === 'Pending' ? 'bg-yellow-50 text-yellow-700 border border-yellow-100' : 'bg-green-50 text-green-700 border border-green-100'
                      }`}>
   ...
   return (
      <div className="flex items-center gap-4 p-3 rounded-xl hover:bg-slate-50 transition-colors">
         ...
         <div>
            ...
            <p className="text-sm font-semibold text-slate-700">{value || "Not set"}</p>
         </div>
      </div>
   );
}

function SkillBar({ label, score, color, width }: { label: string, score: string, color: string, width: string }) {
   return (
      <div>
...
function InputGroup({ label, type = "text", defaultValue, disabled = false }: any) {
   return (
      <div>
